...
@Autonomous(name = "NO PARK BLUE - NJ foundation", group = "Iterative Opmode")

public class NJBlueFoundationNOPARK extends OpMode {
    DcMotor leftFront;
    DcMotor rightFront;
    DcMotor leftBack;
    DcMotor rightBack;
    ...
    driveState turnLeft; //90

    driveState tinyBack;


    driveState strafeOut; //target 30
    ...
    public void init() {
        ...
        lineUP  = new driveState(17, .5, motors, "forward");
        firsTurn = new GyroTurnCCWByPID(80, .5, motors, imu);
        ...
        goBack = new driveState(-25, .5, motors, "backward");

        turnLeft = new driveState(30,.5, motors, "turnLeft");
        ungetFoundation = new DoubleClaspState(foundation, block, 3, 1,0);// comment
        strafeOut = new driveState(10, .3, motors, "right");

        tinyBack = new driveState(-5, .5, motors, "backeard");
...
//        approachSkybridge = new driveState(-30, .5, motors, "backward"); // cpomment
        finals = new ColorSenseStopState(motors, colorSensor, "blue", .5, "backward"); //drives and parks under the bridge
//
//        //State set up
//
        ...
        goBack.setNextState(turnLeft);
        turnLeft.setNextState(ungetFoundation);
        ungetFoundation.setNextState(tinyBack);
        tinyBack.setNextState(null);
